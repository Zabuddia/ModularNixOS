...
dashHtml =
  let
    ...
    mkCard = r:
      let
        ...
        domain = if r.expose == "tailscale" then tsHostLiteral else r.hostLabel;
  ...
  ''
    ...
    <body>
      <div class="wrap">
        ...
        <div class="sub">Quick links to services on this node. Tailscale links resolve at runtime via MagicDNS or TS IP.</div>
        <div class="links">
          <!-- Caddy home (static hostname:443) -->
          <span class="linkchip">Caddy home:
            <a href="https://${config.networking.hostName}:443">https://${config.networking.hostName}:443</a>
          </span>
          <!-- Tailscale home (runtime TS_HOST:basePort) -->
          <span class="linkchip">Tailscale home:
            <a href="https://''${TS_HOST}:${toString basePort}">https://''${TS_HOST}:${toString basePort}</a>
          </span>
        </div>
        <div class="grid">
          ${cards}
  ...
  renderScript = pkgs.writeShellScript "render-expose-dash" ''
    ...
    # Remove trailing dot if present (MagicDNS often ends with ".")
    if [ -n "$TS_DNS" ]; then TS_DNS="''${TS_DNS%.}"; fi
